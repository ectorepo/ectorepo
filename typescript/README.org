...
#+begin_src jq :stdin zodSnips :results output verbatim
. | to_entries
  | map({ group: "zod",
          name: (.key),
...
#+name: asdf
#+headers: :file /tmp/vscodeSnippets.sh
#+begin_src jq :stdin zodSnipsBabel :results output verbatim file :cmd-line -rj
[.[] | @text "printf '\(.snippet)' > ~/.emacs.g/etc/yasnippet/snippets/typescript-mode/\(.prefix).snippet"] | join("\n")
#+end_src
